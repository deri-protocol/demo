...
ABI_DIR = PROJECT_ROOT / "abis"
LOCAL_ENV_PATH = PROJECT_ROOT / ".env"
LOCAL_SESSION_KEY_PATH = PROJECT_ROOT / ".session_key.json"
LOCAL_ACTIVE_PTOKEN_PATH = PROJECT_ROOT / ".active_ptoken.json"
...
def require_active_ptoken_state(
    ...
    if state is None:
        raise RuntimeError("No active pToken is configured. Run `python deri_v5_pro_cli.py init-p-token ...` first.")
...
def zone_payload(zone: ZoneConfig | None) -> dict[str, Any] | None:
    if zone is None:
        return None
    return {
        "key": zone.key,
        "label": zone.label,
        "gateway_address": zone.gateway_address,
    }


def pool_summary_payload(
    pool: PoolMetadata,
    ...
    payload: dict[str, Any] = {
        ...
        "engine_address": pool.engine_address,
        "smart_account_factory": pool.smart_account_factory,
        "symbol_manager": pool.symbol_manager,
        "token_b0": pool.token_b0,
        "vault0": pool.vault0,
        "oracle": pool.oracle,
        "swapper": pool.swapper,
        "iou": pool.iou,
        "total_liquidity": pool.total_liquidity,
        "lps_pnl": pool.lps_pnl,
        "markets": [market_payload(market) for market in pool.markets],
        "symbol_count": len(pool.symbols),
    }
    if zone is not None:
        payload["zone"] = zone_payload(zone)
    if selection_source:
        payload["selection_source"] = selection_source
    return payload


def market_payload(market: MarketMetadata) -> dict[str, Any]:
    return {
        "asset_address": market.asset_address,
        "asset_symbol": market.asset_symbol,
        "asset_decimals": market.asset_decimals,
        "vault": market.vault,
...
def symbol_payload(symbol: SymbolMetadata) -> dict[str, Any]:
    return {
        "symbol": symbol.symbol,
        "symbol_id": symbol.symbol_id,
        "category": symbol.category,
        "alpha": symbol.alpha,
        "funding_period": symbol.funding_period,
        "initial_margin_ratio": symbol.initial_margin_ratio,
        "maintenance_margin_ratio": symbol.maintenance_margin_ratio,
        "min_initial_margin_ratio": symbol.min_initial_margin_ratio,
        "strike_price": symbol.strike_price,
        "is_call": symbol.is_call,
        "min_trade_volume": symbol.min_trade_volume,
    }


def active_ptoken_payload(active_state: ActivePTokenState) -> dict[str, Any]:
    return {
        "account": active_state.account,
        "ichain_chain_id": active_state.ichain_chain_id,
        "dchain_chain_id": active_state.dchain_chain_id,
        "p_token_id": str(active_state.p_token_id),
        "gateway_address": active_state.gateway_address,
        "zone_key": active_state.zone_key,
        "margin_token_address": active_state.margin_token_address,
        "margin_token_symbol": active_state.margin_token_symbol,
        "margin_token_decimals": active_state.margin_token_decimals,
        "created_at": active_state.created_at,
        "updated_at": active_state.updated_at,
    }


def session_key_payload(session_key: SessionKey | None) -> dict[str, Any] | None:
...
def smart_account_status_payload(status: SmartAccountStatus) -> dict[str, Any]:
    return {
        "address": status.address,
        "deployed": status.deployed,
        "session_key": status.session_key,
        "session_key_valid": status.session_key_valid,
    }


def td_account_payload(
    td_account: str | None,
    owner_account: str,
    ...
    return {
        "address": td_account,
        "role": role,
        "matches_owner": td_account.lower() == owner_account.lower(),
        "matches_smart_account": bool(
            smart_account_address and td_account.lower() == smart_account_address.lower()
        ),
    }


def infer_position_category(pool: PoolMetadata | None, symbol_name: str) -> str:
    if pool is not None:
        matches = find_symbol_matches(pool, symbol_name, None)
        if matches:
            return matches[0].category.lower()
    if re.search(r"\b[A-Z]+USD-\d+-(?:C|P)\b", symbol_name, re.IGNORECASE):
        return "option"
    return "futures"


def live_position_payload(position: dict[str, Any], pool: PoolMetadata | None = None) -> dict[str, Any]:
    ...
    if volume > 0:
        ...
        direction = "short"
    symbol_name = str(position["symbol"])
    return {
        "symbol": symbol_name,
        "category": infer_position_category(pool, symbol_name),
        "direction": direction,
        "volume": volume,
        "cost": decimal_or_zero(position.get("cost")),
        "cumulative_funding_per_volume": decimal_or_zero(position.get("cumulativeFundingPerVolume")),
    }


def live_td_state_payload(td_state: dict[str, Any], pool: PoolMetadata | None = None) -> dict[str, Any]:
    positions = [live_position_payload(position, pool=pool) for position in td_state.get("positions", [])]
    open_positions = [position for position in positions if position["direction"] != "flat"]
    return {
        "p_token_id": str(td_state.get("pTokenId")),
        "b_amount": decimal_or_zero(td_state.get("bAmount")),
        "b0_amount": decimal_or_zero(td_state.get("b0Amount")),
        "cumulative_pnl": decimal_or_zero(td_state.get("cumulativePnl")),
        "liquidated": bool(td_state.get("liquidated")),
        "positions": positions,
        "open_positions": len(open_positions),
    }


def engine_position_payload(position: dict[str, int]) -> dict[str, Any]:
    return {
        "volume": from_18(position["volume_18"]),
        "cost": from_18(position["cost_18"]),
        "cumulative_funding_per_volume": from_18(position["cumulative_funding_per_volume_18"]),
    }
...
def parse_pool_metadata(row: dict[str, Any]) -> PoolMetadata:
    ...
    symbols = [
        SymbolMetadata(
            ...
            funding_period=to_decimal(symbol["fundingPeriod"]) if symbol.get("fundingPeriod") not in {None, ""} else None,
            initial_margin_ratio=to_decimal(symbol["initialMarginRatio"]) if symbol.get("initialMarginRatio") not in {None, ""} else None,
            maintenance_margin_ratio=to_decimal(symbol["maintenanceMarginRatio"]) if symbol.get("maintenanceMarginRatio") not in {None, ""} else None,
            min_initial_margin_ratio=to_decimal(symbol["minInitialMarginRatio"]) if symbol.get("minInitialMarginRatio") not in {None, ""} else None,
...
def calculate_intrinsic_value(index_price: Decimal, strike_price: Decimal, is_call: bool) -> Decimal:
    if is_call:
        return max(index_price - strike_price, Decimal("0"))
    return max(strike_price - index_price, Decimal("0"))


def calculate_mark_price_decimal(index_price: Decimal, k: Decimal, traders_net_volume: Decimal) -> Decimal:
    return index_price * (Decimal("1") + k * traders_net_volume)


def calculate_cost_decimal(
    index_price: Decimal,
    k: Decimal,
    traders_net_volume: Decimal,
    trade_volume: Decimal,
) -> Decimal:
    volume_term = (((traders_net_volume + trade_volume) ** 2) - (traders_net_volume**2)) * k / Decimal("2")
    return index_price * (volume_term + trade_volume)


def calculate_unrealized_pnl_decimal(
    volume: Decimal,
    entry_price: Decimal,
    ...
    cost = calculate_cost_decimal(theoretical_price, k, net_volume, -volume)
    return -(entry_price * volume + cost)


def calculate_position_index_pnl_decimal(
    volume: Decimal,
    entry_price: Decimal,
    theoretical_price: Decimal,
) -> Decimal:
    return -(entry_price * volume) + theoretical_price * (-volume)


def calculate_dynamic_maintenance_margin_ratio_decimal(
    delta: Decimal,
    current_index_price: Decimal,
    maintenance_margin_ratio: Decimal,
    gamma: Decimal,
) -> Decimal:
    d_s = current_index_price * maintenance_margin_ratio
    delta_part = abs(delta) * d_s
    gamma_part = abs(gamma) * d_s * d_s / Decimal("2")
    return delta_part + gamma_part


def calculate_futures_open_interest_multiplier_decimal(
    open_volume: Decimal,
    alpha: Decimal,
    ...
    if alpha <= 0 or current_index_price <= 0:
        return Decimal("1")
    open_interest_bound = initial_margin_ratio * liquidity / alpha / current_index_price / Decimal("2")
    if open_interest_bound <= 0:
        return Decimal("1")
    return max(open_volume / open_interest_bound, Decimal("1"))


def calculate_options_open_interest_multiplier_decimal(
    open_volume: Decimal,
    alpha: Decimal,
    initial_margin_ratio: Decimal,
    min_initial_margin_ratio: Decimal,
    delta: Decimal,
    current_index_price: Decimal,
    liquidity: Decimal,
) -> Decimal:
    ...
    if delta == 0:
        adjusted_ratio = initial_margin_ratio
    else:
        adjusted_ratio = max(initial_margin_ratio, min_initial_margin_ratio / abs(delta))
    open_interest_bound = adjusted_ratio * liquidity / alpha / current_index_price / Decimal("2")
    if open_interest_bound <= 0:
        return Decimal("1")
    return max(open_volume / open_interest_bound, Decimal("1"))


def calculate_funding_decimal(
    *,
    current_index_price: Decimal,
    is_option: bool,
    is_call: bool,
    ...
    block_timestamp: int,
    last_timestamp: int,
    funding_period: Decimal,
    symbol_cumulative_funding_per_volume: Decimal,
    volume: Decimal,
    position_cumulative_funding_per_volume: Decimal,
) -> Decimal:
    if funding_period <= 0:
        return Decimal("0")

    subtraction = current_index_price
    diff_funding = current_index_price * Decimal("0.0003") * volume
    if is_option:
        if strike_price is None:
            raise RuntimeError("Option funding calculation requires strike_price")
        subtraction = calculate_intrinsic_value(current_index_price, strike_price, is_call)
        diff_funding = Decimal("0")

    diff = (mark_price - subtraction) * Decimal(block_timestamp - last_timestamp) / funding_period
    funding = volume * ((symbol_cumulative_funding_per_volume + diff) - position_cumulative_funding_per_volume)
    return funding + diff_funding


def build_margin_inputs(
    graph_url: str,
    pool: PoolMetadata,
    symbol: SymbolMetadata,
    symbol_state_cache: dict[str, dict[str, Any]],
    oracle_cache: dict[str, dict[str, Any]],
) -> dict[str, Any]:
    state = symbol_state_cache.get(symbol.symbol_id)
    if state is None:
        fetched = fetch_symbol_state(graph_url, pool.symbol_manager, symbol.symbol_id)
        if fetched is None:
            raise RuntimeError(f"Live symbol state was not found for {symbol.symbol}")
        symbol_state_cache[symbol.symbol_id] = fetched
        state = fetched

    query_symbols: list[str] = []
    if symbol.category.lower() == "option":
        query_price_symbol = get_option_query_price_symbol(symbol)
        query_symbols = [query_price_symbol, f"VOL-{query_price_symbol}"]
    else:
        query_symbols = [symbol.symbol]

    missing = [oracle_symbol for oracle_symbol in query_symbols if oracle_symbol not in oracle_cache]
    if missing:
        oracle_cache.update(fetch_oracles(graph_url, missing))

    if symbol.category.lower() == "option":
        query_price_symbol = get_option_query_price_symbol(symbol)
        price_row = oracle_cache.get(query_price_symbol)
        volatility_row = oracle_cache.get(f"VOL-{query_price_symbol}")
        if price_row is None or volatility_row is None:
            raise RuntimeError(f"Oracle data was not found for {symbol.symbol}")
        current_index_price = decimal_or_zero(price_row.get("value"))
        block_timestamp = int(price_row.get("timestamp") or 0)
        volatility = decimal_or_zero(volatility_row.get("value"))
    else:
        price_row = oracle_cache.get(symbol.symbol)
        current_index_price = decimal_or_zero(price_row.get("value")) if price_row else decimal_or_zero(state.get("lastIndexPrice"))
        block_timestamp = int(price_row.get("timestamp") or 0) if price_row else 0
        volatility = decimal_or_zero(state.get("lastVolatility"))

    last_timestamp = int(state.get("lastTimestamp") or 0) or block_timestamp
    net_volume = decimal_or_zero(state.get("netVolume"))
    open_volume = decimal_or_zero(state.get("openVolume"))
    cumulative_funding_per_volume = decimal_or_zero(state.get("cumulativeFundingPerVolume"))

    if symbol.category.lower() == "futures":
        if symbol.alpha is None or symbol.initial_margin_ratio is None:
            raise RuntimeError(f"Futures symbol metadata is incomplete for {symbol.symbol}")
        k = current_index_price * symbol.alpha / (pool.total_liquidity + pool.lps_pnl)
        theoretical_price = current_index_price
        mark_price = calculate_mark_price_decimal(current_index_price, k, net_volume)
        open_interest_multiplier = calculate_futures_open_interest_multiplier_decimal(
            open_volume,
            symbol.alpha,
            symbol.initial_margin_ratio,
            current_index_price,
            pool.total_liquidity,
        )
        initial_margin_per_volume = current_index_price * symbol.initial_margin_ratio * open_interest_multiplier
        return {
            "current_index_price": current_index_price,
            "block_timestamp": block_timestamp,
            "last_timestamp": last_timestamp,
            "net_volume": net_volume,
            "cumulative_funding_per_volume": cumulative_funding_per_volume,
            "k": k,
            "theoretical_price": theoretical_price,
            "mark_price": mark_price,
            "initial_margin_per_volume": initial_margin_per_volume,
        }

    if symbol.category.lower() == "option":
        if (
            symbol.alpha is None
            or symbol.strike_price is None
            or symbol.initial_margin_ratio is None
            or symbol.maintenance_margin_ratio is None
            or symbol.min_initial_margin_ratio is None
        ):
            raise RuntimeError(f"Option symbol metadata is incomplete for {symbol.symbol}")
        theoretical_price_f, intrinsic_value_f, delta_f, gamma_f, _time_value, _u = calculate_everlasting_option(
            float(current_index_price),
            float(symbol.strike_price),
            float(volatility),
            symbol.is_call,
        )
        theoretical_price = to_decimal(str(theoretical_price_f))
        delta = to_decimal(str(delta_f))
        gamma = to_decimal(str(gamma_f))
        k = to_decimal(
            str(
                calculate_option_k(
                    float(current_index_price),
        ...
        mark_price = calculate_mark_price_decimal(theoretical_price, k, net_volume)
        dynamic_maintenance_margin_ratio = calculate_dynamic_maintenance_margin_ratio_decimal(
            delta,
            current_index_price,
            symbol.maintenance_margin_ratio,
            gamma,
        )
        open_interest_multiplier = calculate_options_open_interest_multiplier_decimal(
            open_volume,
            symbol.alpha,
            symbol.initial_margin_ratio,
            symbol.min_initial_margin_ratio,
            delta,
            current_index_price,
            pool.total_liquidity,
        )
        initial_margin_per_volume = max(
            dynamic_maintenance_margin_ratio * symbol.initial_margin_ratio / symbol.maintenance_margin_ratio,
            current_index_price / Decimal("2000"),
        ...
        return {
            ...
            "mark_price": mark_price,
            "initial_margin_per_volume": initial_margin_per_volume,
            "intrinsic_value": to_decimal(str(intrinsic_value_f)),
        }

    raise RuntimeError(f"Margin info currently supports futures and options only, not {symbol.category}")


def calculate_margin_info(
    graph_url: str,
    pool: PoolMetadata,
    ...
    b0_amount = decimal_or_zero(td_state.get("b0Amount"))
    cumulative_pnl = decimal_or_zero(td_state.get("cumulativePnl"))
    last_cumulative_pnl_on_engine = decimal_or_zero(td_state.get("lastCumulativePnlOnEngine"))
    settled_pnl = cumulative_pnl - last_cumulative_pnl_on_engine

    locked_margin = Decimal("0")
    unsettled_pnl = Decimal("0")
    index_pnl = Decimal("0")
    accumulative_funding = Decimal("0")

    for raw_position in td_state.get("positions", []):
        volume = decimal_or_zero(raw_position.get("volume"))
        if volume == 0:
            continue

        symbol_name = str(raw_position.get("symbol"))
        try:
            symbol = find_symbol(pool, symbol_name, None)
            inputs = build_margin_inputs(graph_url, pool, symbol, symbol_state_cache, oracle_cache)
        except RuntimeError as exc:
            warnings.append(str(exc))
            continue

        cost = decimal_or_zero(raw_position.get("cost"))
        entry_price = abs(cost / volume) if volume != 0 else Decimal("0")
        position_cumulative_funding_per_volume = decimal_or_zero(raw_position.get("cumulativeFundingPerVolume"))

        locked_margin += abs(volume) * inputs["initial_margin_per_volume"]
        unsettled_pnl += calculate_unrealized_pnl_decimal(
            volume,
            entry_price,
            inputs["theoretical_price"],
            inputs["k"],
            inputs["net_volume"],
        )
        index_pnl += calculate_position_index_pnl_decimal(volume, entry_price, inputs["theoretical_price"])
        accumulative_funding += calculate_funding_decimal(
            current_index_price=inputs["current_index_price"],
            is_option=symbol.category.lower() == "option",
            is_call=symbol.is_call,
            strike_price=symbol.strike_price,
            mark_price=inputs["mark_price"],
            block_timestamp=inputs["block_timestamp"],
            last_timestamp=inputs["last_timestamp"],
            funding_period=symbol.funding_period or Decimal("0"),
            symbol_cumulative_funding_per_volume=inputs["cumulative_funding_per_volume"],
            volume=volume,
            position_cumulative_funding_per_volume=position_cumulative_funding_per_volume,
        )

    asset_balance = b0_amount + b_amount
    dynamic_balance = asset_balance + settled_pnl + unsettled_pnl - accumulative_funding
    dynamic_balance_index = asset_balance + settled_pnl + index_pnl - accumulative_funding

    if unsettled_pnl > 0:
        available_before_lock = asset_balance + settled_pnl - accumulative_funding
    else:
        available_before_lock = asset_balance + settled_pnl + unsettled_pnl - accumulative_funding
    available_margin = max(available_before_lock - locked_margin, Decimal("0"))

    margin_usage_ratio = None
    if dynamic_balance_index != 0:
        margin_usage_ratio = locked_margin / dynamic_balance_index * Decimal("100")
    ...
    return {
        ...
        "index_pnl": index_pnl,
        "accumulative_funding": accumulative_funding,
    }, warnings
...
def approve_if_needed(
    ...
    if current >= amount:
        ...
        return {"required": False, "spender": spender, "tx_hash": None}
    ...
    return {"required": True, "spender": spender, "tx_hash": receipt.transactionHash.hex()}
...
def auto_price_limit(
    ...
    return {
        "source": "auto_option_frontend_formula",
        "limit": price_limit,
        "symbol": symbol.symbol,
        "category": symbol.category,
        "index_price": to_decimal(str(current_index_price_float)),
        "mark_price": to_decimal(str(mark_price)),
        "net_volume": to_decimal(str(net_volume_float)),
        "slippage_pct": slippage_pct,
...
def ensure_trading_enabled(
    ...
    timeout: int,
    poll_interval: float,
) -> tuple[SmartAccountStatus, dict[str, Any]]:
    ...
    if (
        ...
        return status, {
            "action": "reused_active_session_key",
            "submitted_enable_trading": False,
            "smart_account": status.address,
            "session_key": status.session_key,
            "deployed": status.deployed,
        }
    ...
    final_status = wait_for_smart_account_status(
        dchain_web3,
    ...
    return final_status, {
        "action": "enabled_trading",
        "submitted_enable_trading": True,
        "smart_account": smart_account_address,
        "session_key": session_key.address,
        "user_op_hash": submitted_user_op_hash,
        "transaction_hash": transaction_hash,
        "deployed_before_submit": status.deployed,
    }
...
def command_init_p_token(args: argparse.Namespace) -> dict[str, Any]:
    account, private_key = resolve_account(args, require_private_key=args.existing_p_token_id is None)
    ...
    if args.existing_p_token_id is not None:
        ...
        return {
            ...
            "pool": pool_summary_payload(pool, zone, selection_source),
            "margin_market": market_payload(market),
            "active_ptoken": {
                **active_ptoken_payload(active_state),
                "state_file": LOCAL_ACTIVE_PTOKEN_PATH,
            },
        }
    ...
    if market.asset_address.lower() != ETH_ADDRESS.lower():
        erc20 = get_contract(ichain_web3, market.asset_address, "Erc20.json")
        approval_result = approve_if_needed(
            erc20,
            account,
    ...
    td_account_info = td_account_payload(bytes32_to_address(td_state[1]), account)
    ...
    return {
        ...
        "pool": pool_summary_payload(pool, zone, selection_source),
        "margin_market": market_payload(market),
        "margin_amount": to_decimal(args.margin_amount),
        "approval": approval_result,
        "request_add_margin": {
            "tx_hash": receipt.transactionHash.hex(),
            "signature": signature,
            "p_token_id": str(p_token_id),
        },
        "td_account": td_account_info,
        "active_ptoken": {
            **active_ptoken_payload(active_state),
            "state_file": LOCAL_ACTIVE_PTOKEN_PATH,
        },
...
def command_status(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if len({pool.smart_account_factory.lower() for pool in pools}) > 1:
        ...
        warnings.append("Pools expose different smart-account factories; using the first pool's factory for status")
    ...
    if active_state is None:
        ...
        if (
            ...
            if td_state_row is None:
                ...
                warnings.append("Active pToken was not found in the live tdState query")
            else:
                active_pool = find_pool_by_gateway(pools, active_state.gateway_address)
                active_live_state = live_td_state_payload(td_state_row, pool=active_pool)
                print(f"  bAmount={format_decimal(active_live_state['b_amount'])}")
                print(f"  b0Amount={format_decimal(active_live_state['b0_amount'])}")
                print(f"  cumulativePnl={format_decimal(active_live_state['cumulative_pnl'])}")
                print(f"  open_positions={active_live_state['open_positions']}")
                active_margin_info, margin_warnings = calculate_margin_info(args.graph_url, active_pool, td_state_row)
                warnings.extend(margin_warnings)
                if active_margin_info:
                    print("  margin_info:")
                    print(f"    dynamicBalance={format_decimal(active_margin_info['dynamic_balance'])}")
                    print(f"    lockedMargin={format_decimal(active_margin_info['locked_margin'])}")
                    print(f"    availableMargin={format_decimal(active_margin_info['available_margin'])}")
        else:
            print("  warning=active pToken does not match the current account or network")
            warnings.append("Active pToken does not match the current account or network")
    ...
    for pool in pools:
        ...
        pool_summaries.append(pool_summary_payload(pool, zone))
    ...
    for market in get_unique_markets(pools):
        ...
        balances.append({"market": market_payload(market), "balance": balance})
    ...
    discovered_existing = None
    if active_state is None and len(td_states) == 1:
        discovered_existing = str(td_states[0]["pTokenId"])
        print("  discovered_existing_pTokenId=" + discovered_existing)
        print(
            "  note=Run `python deri_v5_pro_cli.py init-p-token --existing-p-token-id ...` "
            "to adopt it as the active demo pToken."
        )
    result: dict[str, Any] = {
        "resolved": {
            "ichain_chain_id": ichain_chain_id,
        ...
        "account": {
            "owner": account,
            "smart_account": smart_account_status_payload(smart_account_status),
            "local_session_key": session_key_payload(local_session_key),
        },
        "active_ptoken": None
        if active_state is None
        else {
            **active_ptoken_payload(active_state),
            "state_file": LOCAL_ACTIVE_PTOKEN_PATH,
            "live_state": active_live_state,
            "margin_info": active_margin_info,
        },
        "pools": pool_summaries,
        "balances": balances,
        "portfolio": {
            "td_states": len(td_states),
            "open_positions": open_positions,
            "discovered_existing_p_token_id": discovered_existing,
        },
    }
    if warnings:
...
def command_positions(args: argparse.Namespace) -> dict[str, Any]:
    account, _private_key = resolve_account(args, require_private_key=False)
    ...
    if td_state is None:
        ...
        return {
            ...
            "pool": pool_summary_payload(pool, zone, selection_source),
            "active_ptoken": active_ptoken_payload(active_state),
            "live_state": None,
            "positions": [],
        }

    live_state = live_td_state_payload(td_state, pool=pool)
    print(f"  bAmount={format_decimal(live_state['b_amount'])}")
    print(f"  b0Amount={format_decimal(live_state['b0_amount'])}")
    print(f"  cumulativePnl={format_decimal(live_state['cumulative_pnl'])}")
    print(f"  liquidated={live_state['liquidated']}")

    positions = live_state["positions"]
    if args.symbol:
        positions = [position for position in positions if position["symbol"].upper() == args.symbol.upper()]
    ...
    for position in positions:
        print(
            ...
            f"volume={format_decimal(position['volume'])}",
            f"cost={format_decimal(position['cost'])}",
            f"cumulativeFundingPerVolume={format_decimal(position['cumulative_funding_per_volume'])}",
        )
    return {
        "resolved": {
            "ichain_chain_id": ichain_chain_id,
...
def command_add_margin(args: argparse.Namespace) -> dict[str, Any]:
    ...
    td_account_info = td_account_payload(bytes32_to_address(td_state[1]), account)
    print_td_account_hint(bytes32_to_address(td_state[1]), account)
    updated_state = build_active_ptoken_state(
        account=account,
        ichain_chain_id=ichain_chain_id,
        dchain_chain_id=dchain_chain_id,
        p_token_id=active_state.p_token_id,
    ...
    return {
        ...
        "pool": pool_summary_payload(pool, zone, selection_source),
        "active_ptoken": active_ptoken_payload(updated_state),
        "margin_market": market_payload(market),
        "margin_amount": to_decimal(args.margin_amount),
        "approval": approval_result,
        "request_add_margin": {
...
def run_trade_command(args: argparse.Namespace, close_position: bool) -> dict[str, Any]:
    account, private_key = resolve_account(args, require_private_key=True)
    ...
    smart_account_status, bootstrap_result = ensure_trading_enabled(
    ...
    td_account_info = td_account_payload(td_account, account, smart_account_status.address)
    ...
    if args.price_limit:
        price_limit_result = {
            "source": "manual",
            "limit": to_decimal(args.price_limit),
        }
    else:
        price_limit_result = auto_price_limit(
            args.graph_url,
            symbol_manager,
            pool,
            symbol,
    ...
    price_limit = price_limit_result["limit"]
    ...
    return {
        ...
        "trade_volume": from_18(trade_volume_18),
        "td_account": td_account_info,
        "smart_account": smart_account_status_payload(smart_account_status),
        "session_key": session_key_payload(session_key),
        "bootstrap": bootstrap_result,
        "initial_position": engine_position_payload(initial_position),
        "price_limit": price_limit_result,
        "trade_submission": {
            "user_op_hash": user_op_hash,
            "transaction_hash": trade_tx_hash,
        },
        "final_position": engine_position_payload(final_position),
    }


def command_remove_margin(args: argparse.Namespace) -> dict[str, Any]:
    account, private_key = resolve_account(args, require_private_key=True)
    ichain_web3, dchain_web3, ichain_chain_id, dchain_chain_id = connect_web3_clients(args)
    pools = fetch_pool_metadatas(args.graph_url, ichain_chain_id)
    active_state = require_active_ptoken_state(account, ichain_chain_id, dchain_chain_id)
    ...
    print(f"  smartAccount={smart_account_address}")
    print(f"  deployed={smart_account_status.deployed}")
    td_account_info = td_account_payload(bytes32_to_address(td_state[1]), account, smart_account_address)
    ...
    updated_td_state_payload = {
        "b_amount": from_18(int(updated_td_state[3])),
        "b0_amount": from_18(int(updated_td_state[4])),
    }
    print("Updated dChain tdState:")
    print(f"  bAmount={format_decimal(updated_td_state_payload['b_amount'])}")
    print(f"  b0Amount={format_decimal(updated_td_state_payload['b0_amount'])}")
    ...
    return {
        ...
        "active_ptoken": active_ptoken_payload(updated_state),
        "margin_market": market_payload(market),
        "remove_mode": "all" if args.all else "partial",
        "remove_amount": None if args.all else to_decimal(args.margin_amount),
        "smart_account": smart_account_status_payload(smart_account_status),
        "td_account": td_account_info,
        "wallet_balance_before": wallet_balance_before,
        "remove_margin_submission": {
            "user_op_hash": user_op_hash,
            "transaction_hash": remove_tx_hash,
        },
        "updated_td_state": updated_td_state_payload,
        "wallet_balance_after": settled_balance,
        "wallet_settlement_observed": settled_balance is not None,
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "    python deri_v5_pro_cli.py init-p-token --zone main --margin-token USDC --margin-amount 10\n\n"
            "  Trade the active pToken:\n"
            "    python deri_v5_pro_cli.py trade --symbol BTCUSD --trade-volume 0.0001\n\n"
            "  Close an existing position:\n"
            "    python deri_v5_pro_cli.py close-position --symbol BTCUSD\n\n"
            "  Remove margin from the active pToken:\n"
            "    python deri_v5_pro_cli.py remove-margin --margin-amount 5\n\n"
...
def dispatch_command(args: argparse.Namespace) -> dict[str, Any]:
    if args.command == "init-p-token":
        return command_init_p_token(args)
    if args.command == "status":
        return command_status(args)
    if args.command == "positions":
        return command_positions(args)
    if args.command == "add-margin":
        return command_add_margin(args)
    if args.command == "trade":
        return run_trade_command(args, close_position=False)
    if args.command == "close-position":
        return run_trade_command(args, close_position=True)
    if args.command == "remove-margin":
        return command_remove_margin(args)
